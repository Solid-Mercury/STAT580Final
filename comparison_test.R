...
##########################This is method 1 #######################
  ...
  for (j in 1:length(lambda)){
    teste_temp[j] <- trainingerror(z[[i]],sim.matrix[[j]], omega[[i]])
  }
...
df2 <- data.frame(time1 = time_save1/50,time2 = time_save2/50,time3 = time_save3/50,time4 = time_save4/50,error1 = teste_save1/50,error2 = teste_save2/50,
                  error3 = teste_save3/50,error4 = teste_save4/50,truerank = rank_save, SNR = SNR_save, p = p_save)
#df2 <- data.frame(time1 = time_save1,time2 = time_save2,time3 = time_save3,error1 = teste_save1,error2 = teste_save2,
#                  error3 = teste_save3,truerank = rank_save, SNR = SNR_save, p = p_save)
...
df2
net <- matrix(scan("C:\\R\\Movie data\\trainng.txt"), nrow = 943)
net[is.na(net)] <- 0
svd(net)$d
